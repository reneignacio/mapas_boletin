...
for veg_index in veg_index_:
    ...
    for region in regiones.keys():
        R = region
        ruta_diferencia_tif = "data\\{0}\\TIF\\DIFERENCIA\\{0}_Dif_{1}_ZA.tif".format(R,veg_index)
    ...
    # Filtrar y almacenar los nombres de las capas que contienen "Dif_NDVI o SAVI"
    diferencia_layers = [name for name in layer_names if "Dif_{}".format(veg_index) in name]
    # Contar y mostrar los nombres de las capas
    diferencia_count = len(diferencia_layers)
    print(diferencia_count)
    print(diferencia_layers)
    ...
    def aplicar_simbologia(region):
        capa_destino = "{}_Dif_{}_ZA.tif".format(region,veg_index)
    ...
    def proceso(region):
        ...
        capas.extend(["{}_Dif_{}_ZA.tif".format(region,veg_index), "Lagos_{}".format(region)])
    ...
    # Asegurarse de que la capa "Regional" está visible
        ...
        salida_png = os.path.join(carpeta, "{}_DIFERENCIA_{}.png".format(region,veg_index))
...
#FALTA CAMBIAR .lyr para el tif de diferenciam ya que esta el de anomalia
