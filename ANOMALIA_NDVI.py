...
legend.autoAdd = False
# Cargar capa NDVI.tif
for region in regiones.keys():
    R = region
    ruta_anomalia_NDVI_tif = "data\\{0}\\TIF\\ANOMALIA\\{0}_Anomalia_NDVI_median_ZA.tif".format(R)
    if os.path.exists(ruta_anomalia_NDVI_tif):
        try:
            capa_anomalia = arcpy.mapping.Layer(ruta_anomalia_NDVI_tif)
            arcpy.mapping.AddLayer(df, capa_anomalia, "BOTTOM")
            capa_anomalia.visible=False
            print("tif {} añadido".format(R))
        except Exception as e:
            print("Error cargando capa NDVI para la region {}: {}".format(R, e))
...
# Filtrar y almacenar los nombres de las capas que contienen "Anomalia_NDVI"
anomalia_NDVI_layers = [name for name in layer_names if "Anomalia_NDVI" in name]
# Contar y mostrar los nombres de las capas
anomalia_NDVI_count = len(anomalia_NDVI_layers)
print(anomalia_NDVI_count)
print(anomalia_NDVI_layers)
#NDVI AÑADIDO
...
def proceso(region):
    ...
    capas.extend(["{}_Anomalia_NDVI_median_ZA.tif".format(region), "Lagos_{}".format(region)])
...
# Asegurarse de que la capa "Regional" está visible
    ...
    # Crear subcarpeta NDVI
    carpeta_NDVI = os.path.join(carpeta_region, "NDVI")
    if not os.path.exists(carpeta_NDVI):
        os.makedirs(carpeta_NDVI)
